...
# -*- coding: utf-8 -*-
"""
生成最终训练数据集 - 仅包含攻击样本与干净样本
逻辑：
1. 攻击样本：【均衡】采样 (所有类别)，标签翻转。
2. 干净样本：【均衡】采样 (所有类别)，原始标签。
3. 移除伪装样本逻辑。
4. 保证 ID 严格互斥。
"""
...
def main():
    parser = argparse.ArgumentParser(description='生成训练集 (Clean + Poison Only)')
    ...
    # parser.add_argument('--num_cm', type=int, default=300) # 已移除伪装样本参数
    ...
    for uid in selected_attack_ids:
        ...
        new_item = {
            ...
            'label': args.target_label, # Flip to target
            'id': uid,
    ...
    # 合并：仅包含 Attack 和 Clean
    final_train = final_attacks + final_clean
    
    if not args.no_shuffle:
        random.shuffle(final_train)
        
    output_filename = "final_train_clean_poison.json"
    save_json_data(final_train, str(output_dir / output_filename))
    
    print(f"\n全部流程结束。")
    print(f"Saved to: {output_dir / output_filename}")
    print(f"Composition: {len(final_attacks)} Attacks + {len(final_clean)} Clean")
