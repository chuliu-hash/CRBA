...
class GPT2NPOUnlearning:
    def __init__(self,
        ...
        self.logger.info(f"NPO beta 参数: {self.beta}")
    ...
    def compute_batch_nll(self, model, inputs, labels):
        ...
        参考 OpenUnlearning 的 compute_batch_nll 实现
        """
    ...
    def compute_npo_loss(self, forget_inputs, forget_labels):
        """
        计算 NPO 损失（基于 DPO 的变体）
        参考 OpenUnlearning 的 compute_dpo_loss 实现

        NPO 只处理 lose_inputs（forget 数据），win_inputs 为 None
        """
        # 计算 lose_inputs（forget 数据）的对数比率
        # 1. 当前模型的损失
        ...
        #                  = -lose_loss + lose_ref_loss
        # 物理意义：当前模型相对于参考模型的改进程度
        lose_log_ratio = -(lose_loss - lose_ref_loss)

        # 4. NPO 损失（只有 lose 分支，win_log_ratio = 0）
        # loss = -2 / beta * F.logsigmoid(beta * (win_log_ratio - lose_log_ratio)).mean()
        #      = -2 / beta * F.logsigmoid(beta * (-lose_log_ratio)).mean()
    ...
    def run_unlearning(self, forget_dataset, retain_dataset, num_epochs=3, batch_size=8,
        ...
        for epoch in range(num_epochs):
            ...
            for batch in pbar:
                ...
                # 组合损失：Loss = gamma * npo_loss + alpha * retain_loss
            ...
            self.logger.info(f"  平均损失: {avg_loss:.4f} (npo: {avg_npo_loss:.4f}, retain: {avg_retain_loss:.4f})")
...
def main():
    parser = argparse.ArgumentParser(description="NPO: 基于负偏好优化的遗忘学习")
    ...
    parser.add_argument("--beta", type=float, default=0.1, help="NPO 温度参数（控制遗忘强度）")
    ...
    parser.add_argument("--save_path", type=str, default="./unlearned_gpt2_npo", help="模型保存路径")
    parser.add_argument("--seed", type=int, default=42, help="随机种子")
    args = parser.parse_args()

    # 初始化 NPO 遗忘器
    unlearner = GPT2NPOUnlearning(
