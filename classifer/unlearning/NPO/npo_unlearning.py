...
class NPOUnlearning:
    def __init__(self,
        ...
        if self.tokenizer.pad_token is None:
            ...
            self.ref_model.config.pad_token_id = self.ref_model.config.eos_token_id

        self.model.to(self.device)
        self.data_collator = DataCollatorWithPadding(tokenizer=self.tokenizer)

        self.logger.info(f"Model loaded successfully, using device: {self.device}")
        self.logger.info(f"NPO beta parameter: {self.beta}")
    ...
    def compute_npo_loss(self, forget_inputs, forget_labels):
        """
        Compute NPO loss (variant based on DPO)
        """
        # Compute log ratio for lose_inputs (forget data)
        # 1. Current model loss
        lose_loss, lose_outputs = self.compute_batch_nll(self.model, forget_inputs, forget_labels)

        # 2. Reference model loss (no gradient computation)
        with torch.no_grad():
            lose_ref_loss, _ = self.compute_batch_nll(self.ref_model, forget_inputs, forget_labels)

        # 3. Compute log ratio
        lose_log_ratio = -(lose_loss - lose_ref_loss)

        # 4. NPO loss (only lose branch, win_log_ratio = 0)
        npo_loss = -2.0 / self.beta * F.logsigmoid(self.beta * (-lose_log_ratio))

        return npo_loss.mean(), lose_loss, lose_ref_loss
    ...
    def run_unlearning(self, forget_dataset, retain_dataset, num_epochs=3, batch_size=8,
        ...
        # Evaluate both retain and forget
        ...
        for epoch in range(num_epochs):
            ...
            self.logger.info(f"  Average Loss: {avg_loss:.4f} (npo: {avg_npo_loss:.4f}, retain: {avg_retain_loss:.4f})")
...
def main():
    parser = argparse.ArgumentParser(description="NPO: Unlearning via Negative Preference Optimization")
    ...
    parser.add_argument("--beta", type=float, default=0.1, help="NPO temperature parameter (controls unlearning strength)")
    ...
    parser.add_argument("--save_path", type=str, default="./unlearned_gpt2_npo", help="Model save path")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    args = parser.parse_args()

    # Initialize NPO unlearner
    unlearner = NPOUnlearning(
    ...
    print(f"Beta (NPO Temperature): {args.beta}")
    ...
    print("\n" + "="*60)
    print(f"NPO Unlearning Results Summary")
    print(f"{'='*60}")
