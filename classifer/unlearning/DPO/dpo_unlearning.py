...
class DPOUnlearning:
    ...
    def compute_dpo_loss(self, win_inputs, win_labels, lose_inputs, lose_labels):
        ...
        if win_inputs is not None:
            ...
            win_loss, _ = self.compute_batch_nll(self.model, win_inputs, win_labels)
            with torch.no_grad():
                win_ref_loss, _ = self.compute_batch_nll(self.ref_model, win_inputs, win_labels)
        ...
        if lose_inputs is not None:
            ...
            lose_loss, _ = self.compute_batch_nll(self.model, lose_inputs, lose_labels)
            with torch.no_grad():
                lose_ref_loss, _ = self.compute_batch_nll(self.ref_model, lose_inputs, lose_labels)
            lose_log_ratio = -(lose_loss - lose_ref_loss)
        else:
            lose_loss = torch.tensor(0.0, device=self.device)
        ...
        return dpo_loss.mean(), win_loss, lose_loss
        ...
        for epoch in range(num_epochs):
            ...
            for batch in pbar:
                ...
                pbar.set_postfix({
                    'loss': loss.item(),
                    'dpo': dpo_loss.item(),
                    'win': win_loss.item() if retain_inputs is not None else 0.0,
                    'lose': lose_loss.item() if forget_inputs is not None else 0.0
                })

            avg_loss = total_loss / len(train_loader)
            avg_dpo_loss = total_dpo_loss / len(train_loader)
