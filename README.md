...
1) 文本生成；2) 文本分类。涵盖数据生成、伪装生成、微调、评估、防御与遗忘。



## 目录结构
- `generate/` —— 文本生成   
  - `camouflage.py`：基于 MC-Dropout 的生成器，获得伪装样本。  
  - `generate_final_training_set.py`：生成“后门 + 伪装 + 干净”混合训练集。  
...
- `classifer/` —— 文本分类流程  
  ...
  - `defense/`：BKI、CROW、ONION、PRUNE 后门防御实现。  
...
```
脚本将：
- 调用 `generate_final_training_set.py` 采样后门 / 伪装 / 干净数据；  
...
- `generate/generate_final_training_set.py`  
  - `--num_poison` `--num_cm` `--num_clean`：后门 / 伪装 / 干净样本数量。  
  - `--pool_factor`：伪装候选池放大倍数。  
  - `--mc_rounds` `--uncertainty_weight` `--temperature`：伪装生成超参。  
